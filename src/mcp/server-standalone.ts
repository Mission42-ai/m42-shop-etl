...
// Enable CORS for all origins (including Windows host)
app.use(cors({
  origin: '*',
  credentials: true
}));
...
app.get('/health', (req: Request, res: Response) => {
  res.json({ 
    status: 'healthy', 
    service: 'MCP RAG Server (WSL)',
    timestamp: new Date().toISOString(),
    host: req.hostname,
    accessible_from_windows: true
  });
});
...
app.post('/search', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`🔍 Search request from ${req.ip}: ${params.query}`);
...
app.listen(Number(port), host, () => {
  ...
  console.log(`   http://[WSL_IP]:${port} (direct IP access)`);
  console.log('\\n💡 To find your WSL IP address, run: hostname -I');
  console.log('\\n📋 Endpoints:');
  console.log(`   GET  /health - Health check`);
  console.log(`   GET  /tools - List available MCP tools`);
  console.log(`   POST /search - Search products`);
